        ...
        <div class="container-fluid px-4 px-lg-6 mt-4">
            ...
            foreach ($shoesproducts as $shoesproduct) {
                echo '<div id="card" class="col mb-3">
                        <a href="index.php?page=main&controller=detail&id=' . $shoesproduct->id . '&action=index" class="card h-100 text-decoration-none">';
                
                if ($shoesproduct->sale) {
                    echo '<div class="badge bg-dark text-light position-absolute" style="top: 0.5rem; right: 0.5rem"> - ' . $shoesproduct->sale . '%</div>';
                }
                
                echo '<img class="card-img-top" src="' . $shoesproduct->img . '" alt="..." style="height: 80%; width: auto;">
                        <div class="card-body p-4">
                            <div class="text-center">
                                <p class="product-name text-muted" style="font-size: 1em">' . $shoesproduct->name . '</p>
                                <span class="fw-bold">' . number_format($shoesproduct->price * (100 - $shoesproduct->sale) / 100, 0, ',', '.') . ' đ</span>
                                <span class="text-muted text-decoration-line-through">' . number_format($shoesproduct->price, 0, ',', '.') . ' đ</span>';
            
                if ($shoesproduct->vote_number == 0) {
                    echo '<p class="product-name text-muted" style="font-size: 1em">' . $shoesproduct->vote_number . ' lượt đánh giá</p>';
                } else {
                    echo '<p class="product-name text-muted" style="font-size: 1em">' . $shoesproduct->vote_number . ' lượt đánh giá</p>
                          <p class="product-name text-muted" style="font-size: 1em">' . $shoesproduct->total_stars / $shoesproduct->vote_number . ' đánh giá trung bình</p>';
                }
